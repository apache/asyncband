...
#[test]
fn assigned_signals_are_fifo_and_cannot_be_stolen() {
    let event = AutoResetEvent::new();
    ...
    // A second set serves the next waiter even though the first has not been polled again.
    event.set();
...
#[test]
fn cancelling_unselected_waits_preserves_fifo_without_adding_a_signal() {
    let event = AutoResetEvent::new();
    let mut first = Box::pin(event.wait());
    ...
    assert!(poll_once(last.as_mut()).is_pending());
    event.set();
    assert!(poll_once(last.as_mut()).is_ready());
}
