...
fn unbounded_collects_from_multiple_producers() {
    let (tx, mut rx) = mpsc::unbounded();
    let messages = if cfg!(miri) { 32 } else { 128 };
    ...
    thread::scope(|scope| {
        ...
        start.wait();
        let mut next = [0; 4];
        while let Ok((producer, sequence)) = pollster::block_on(rx.recv()) {
            assert_eq!(sequence, next[producer]);
            next[producer] += 1;
        }
        assert_eq!(next, [messages; 4]);
    });
}

#[test]
fn unbounded_first_publication_racing_with_close_drops_every_payload_once() {
    ...
    let messages = if cfg!(miri) { 4 } else { 128 };
    for _ in 0..if cfg!(miri) { 2 } else { 16 } {
        let (tx, rx) = mpsc::unbounded();
        let drops: Vec<_> = (0..4 * messages)
            .map(|_| Arc::new(AtomicUsize::new(0)))
            .collect();
        let start = Barrier::new(5);
        thread::scope(|scope| {
            for values in drops.chunks(messages) {
                let tx = tx.clone();
                let start = &start;
                scope.spawn(move || {
                    start.wait();
                    for count in values {
                        drop(tx.send(Payload(count.clone())));
                    }
                });
            }
            start.wait();
