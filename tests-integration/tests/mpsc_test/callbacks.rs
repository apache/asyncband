...
#[test]
fn unbounded_disconnect_drops_buffered_values_outside_lock() {
...
fn unbounded_disconnect_finishes_cleanup_after_a_payload_panics() {
    struct Payload {
        id: usize,
        drops: Arc<[AtomicUsize; 40]>,
    }
    impl Drop for Payload {
        fn drop(&mut self) {
            self.drops[self.id].fetch_add(1, Ordering::Relaxed);
            assert_ne!(self.id, 2, "payload destructor panic");
        }
    }
    let drops = Arc::new(std::array::from_fn(|_| AtomicUsize::new(0)));
    ...
    assert!(std::panic::catch_unwind(|| drop(rx)).is_err());
    assert!(drops.iter().all(|count| count.load(Ordering::Relaxed) == 1));
    assert!(tx.send(Payload { id: 39, drops }).is_err());
}
