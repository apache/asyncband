...
* Reject bounded MPSC capacities above `usize::MAX >> 1` up front with an explicit panic message instead of an opaque arithmetic overflow; zero-sized messages need no slot storage and remain limited only by the permit counter.
