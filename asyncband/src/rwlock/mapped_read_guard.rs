...
pub fn new<'a, T: ?Sized>(
    ...
    MappedRwLockReadGuard { data, access }
}
...
impl<'a, T: ?Sized> MappedRwLockReadGuard<'a, T> {
    ...
    /// Call this as `MappedRwLockReadGuard::map(guard, project)` to avoid shadowing methods of the
    /// value.
    ...
    /// `MappedRwLockReadGuard::filter_map(guard, project)`.
