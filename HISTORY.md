...
- `waitgroup::WaitGroup` adopts the coordinator and worker roles from [`waitgroup-rs`](https://github.com/laizy/waitgroup-rs), but uses Asyncband's internal `CountdownState` to provide cancellable multi-observer waits and explicit completion synchronization.
